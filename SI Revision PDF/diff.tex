...
%DIF DEL SI_old.tex       Sun Dec  1 13:21:09 2019
%DIF ADD SI_revised.tex   Sun Dec  1 18:15:46 2019
...
\DIFdelbegin \DIFdel{In the main paper, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend describe a method for analyzing survey data and search history for real-time tracking of Influenza-like Illness (ILI). This supporting document outlines in detail the steps we took to implement the survey, analyze it, and build a flu-tracking model from it. We first describe the process for labeling queries according to whether they reflect an underlying flu-like experience. We use \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend labeled searches and survey responses to discern differences between users in the onset of ILI symptoms and flu search behavior. We then use insights from this analysis to build an aggregate-level flu-tracking model. 

\section{Sources of Data\DIFdelbegin \DIFdel{:}\DIFdelend }
Data from this study come from the following sources:
\begin{itemize}
\item Survey data from a panel individual users coupled with \DIFaddbegin \DIFadd{Internet }\DIFaddend search and browsing history
\item Census data on demographic distributions at the zip code level \DIFaddbegin \DIFadd{(largely from the American Community Survey)
}\DIFaddend \item Daily anonymized search queries at the zip code level from the Bing search engine
\item Weekly flu reports from the Centers for Disease Control (CDC)
\end{itemize}

\section{\DIFdelbegin \DIFdel{Additional Panel }\DIFdelend Survey \DIFdelbegin \DIFdel{Information}\DIFdelend \DIFaddbegin \DIFadd{Recruitment and Fielding}\DIFaddend }


\subsection{\DIFdelbegin \DIFdel{Attributes of }\DIFdelend \DIFaddbegin \DIFadd{Source Vendor }\DIFaddend Panel \DIFdelbegin \DIFdel{Survey}\DIFdelend \DIFaddbegin \DIFadd{and Sampling}\DIFaddend }
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We partnered with a survey vendor, Luth Research, who works \DIFdelbegin \DIFdel{on an ongoing basis }\DIFdelend \DIFaddbegin \DIFadd{periodically }\DIFaddend with Microsoft Research conducting opinion research. Their ongoing panel includes approximately 20,000 individuals with personal computers  \footnote{Luth Research conducted recruitment for the panel, adheres to a strict privacy policy, and complies with ISO standards of data security and HIPPA regulations.}. \DIFdelbegin \DIFdel{These individuals are considered by the vendor to be broadly representative of the U. S. public. }\DIFdelend \DIFaddbegin \DIFadd{Luth research recruits users based on a quota system to construct a panel that is representative of Internet-connected US adults. }\DIFaddend All individuals agreed to participate in marketing research in return for monetary compensation. This agreement included installing a program to track their web browsing and search activities while also responding to questionnaires. All survey and panel data are purged of any personally identifiable information before they are received for analysis.  
...
In total we had 1,180 in the search group and 4,000 in the non-search group. After this matching, there was no statistically significant difference in mean search volume between the groups (p = 0.77). From these individuals, we collected demographic information and collected survey results \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend a total of 654 individuals, of which \DIFdelbegin \DIFdel{11 }\DIFdelend \DIFaddbegin \DIFadd{10 }\DIFaddend did not have any reported search volume in the sample period (see table \ref{tab: Descriptives}). This left us with 262 who had searched a flu-related keyword or site and 382 who did not \DIFaddbegin \DIFadd{(omitting the 10 with zero search volume)}\DIFaddend . 
...
\DIFaddbegin \subsection{\DIFadd{Survey Fielding Timeline}}
\DIFaddend We fielded our flu survey in the spring of 2015. \DIFaddbegin \DIFadd{We fielded a first wave of the survey from March 19th to March 27th 2015, then followed up with a second wave in the field from April 27th to April 31 to supplement sample size. This field period was largely after the flu season had peaked - the flu season typically peaks in February, but sometimes peaks in December, January, and March. During the 2014-2015 season, the flu peaked in December }\url{https://www.cdc.gov/flu/pastseasons/1415season.htm}\DIFadd{. We collected accompanying browsing data when a user completed the survey. The earliest query or page visit containing key words associated with flu was 10-31-2014 and the latest was 3-31-15. 
}

\subsection{\DIFadd{Survey Questions}}

\DIFaddend Our survey questionnaire asked respondents for demographic, household, and flu-related information. With regard to the flu, we were interested in whether respondents, or a member of their household, recently had a flu-like experience, which we inferred from a combination of \DIFdelbegin \DIFdel{symptoms, including fever and cough}\DIFdelend \DIFaddbegin \DIFadd{self-reported symptoms}\DIFaddend . We asked respondents about all symptoms of influenza-like illness since November 2015, followed by a question \DIFdelbegin \DIFdel{in which we ask in which }\DIFdelend \DIFaddbegin \DIFadd{about which }\DIFaddend month these were experienced (see wording below). We repeated these questions separately \DIFaddbegin \DIFadd{for different members of the household}\DIFaddend , asking about children and other adults in the household (including spouses). \DIFdelbegin \DIFdel{We further }\DIFdelend \DIFaddbegin \DIFadd{Respondents indicating they experienced both fever and cough were labeled as having a flu-like experience (though we also tested our models using a label of fever + cough or fever + sore throat as the label, see section 6.4). 
}

\DIFadd{We then }\DIFaddend followed up by asking which sources\footnote{Health care provider, health website, search engine, book, friend or none} (if any) \DIFdelbegin \DIFdel{these individuals }\DIFdelend \DIFaddbegin \DIFadd{respondents }\DIFaddend used when seeking information about these symptoms and health care provider diagnoses (if relevant). %We then compared the survey response results between those who searched for a flu related term and those who did not.
...
\hline
 & \DIFaddFL{Household flu }& \DIFaddFL{Respondent flu }& \DIFaddFL{Spouse flu }\\
\hline
\DIFaddFL{(Intercept)         }& \DIFaddFL{$-0.82$       }& \DIFaddFL{$-1.05$       }& \DIFaddFL{$-1.13$     }\\
                    & \DIFaddFL{$(0.53)$      }& \DIFaddFL{$(0.57)$      }& \DIFaddFL{$(0.74)$    }\\
\DIFaddFL{Volume              }& \DIFaddFL{$0.05$        }& \DIFaddFL{$0.10$        }& \DIFaddFL{$0.03$      }\\
                    & \DIFaddFL{$(0.06)$      }& \DIFaddFL{$(0.06)$      }& \DIFaddFL{$(0.08)$    }\\
\DIFaddFL{Female              }& \DIFaddFL{$0.24$        }& \DIFaddFL{$0.08$        }& \DIFaddFL{$-0.45$     }\\
                    & \DIFaddFL{$(0.19)$      }& \DIFaddFL{$(0.20)$      }& \DIFaddFL{$(0.24)$    }\\
\DIFaddFL{Parent              }& \DIFaddFL{$1.34^{***}$  }& \DIFaddFL{$0.53^{**}$   }& \DIFaddFL{$0.25$      }\\
                    & \DIFaddFL{$(0.18)$      }& \DIFaddFL{$(0.19)$      }& \DIFaddFL{$(0.24)$    }\\
\DIFaddFL{Age                 }& \DIFaddFL{$-0.23^{***}$ }& \DIFaddFL{$-0.23^{***}$ }& \DIFaddFL{$-0.19^{*}$ }\\
                    & \DIFaddFL{$(0.07)$      }& \DIFaddFL{$(0.07)$      }& \DIFaddFL{$(0.09)$    }\\
\DIFaddFL{Early Response      }& \DIFaddFL{$0.39$        }& \DIFaddFL{$0.19$        }& \DIFaddFL{$0.51$      }\\
                    & \DIFaddFL{$(0.24)$      }& \DIFaddFL{$(0.25)$      }& \DIFaddFL{$(0.34)$    }\\
\hline
\DIFaddFL{AIC                 }& \DIFaddFL{758.25        }& \DIFaddFL{701.23        }& \DIFaddFL{471.15      }\\
\DIFaddFL{BIC                 }& \DIFaddFL{785.05        }& \DIFaddFL{728.03        }& \DIFaddFL{496.52      }\\
\DIFaddFL{Log Likelihood      }& \DIFaddFL{-373.12       }& \DIFaddFL{-344.61       }& \DIFaddFL{-229.58     }\\
\DIFaddFL{Deviance            }& \DIFaddFL{746.25        }& \DIFaddFL{689.23        }& \DIFaddFL{459.15      }\\
\DIFaddFL{Num. obs.           }& \DIFaddFL{644           }& \DIFaddFL{644           }& \DIFaddFL{507         }\\
\hline
\multicolumn{4}{l}{\scriptsize{$^{***}p<0.001$, $^{**}p<0.01$, $^*p<0.05$}}
\end{tabular}
\caption{\DIFaddFL{Analysis comparing early responders with late responders in reported flu-like symptoms. The outcome variable is self-reported flu-like experience in the household, for the respondent, and for a spouse.}}
...
\subsubsection*{\DIFadd{Survey Question Variables and Wording}}

\DIFadd{This section presents the survey response options and survey questions asked. 
}

\textbf{\DIFadd{Demographics}}

\DIFaddend \emph{Gender}
\begin{itemize}
\item Male
\item Female
\end{itemize}
...
\item some college
\item \DIFdelbegin \DIFdel{associate?}\DIFdelend \DIFaddbegin \DIFadd{associateâ}\DIFaddend s degree
...
\DIFdelbegin \subsection*{\DIFdel{Labeling Scheme}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Labeling Scheme}}
\DIFaddend 
...
\DIFdelbegin \subsection*{\DIFdel{Codebook: Descriptions of Labels}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Codebook: Descriptions of Labels}}
\DIFaddend \begin{itemize}
\item A1. Related to experienced ILI \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend symptoms of canonical flu definition (cough or fever [can be in combination with other symptoms), treatments for flu, generic flu searches/pages (as if from someone searching re how to diagnose or treat).

Examples: searches - \DIFdelbegin \DIFdel{?flu?, ?cough?, ?}\DIFdelend \DIFaddbegin \DIFadd{â€œfluâ€, â€œcoughâ€, â}\DIFaddend what to do if you have the \DIFdelbegin \DIFdel{flu?, ?}\DIFdelend \DIFaddbegin \DIFadd{fluâ€, â}\DIFaddend productive cough fever body \DIFdelbegin \DIFdel{aches?, ?tamiflu information?}\DIFdelend \DIFaddbegin \DIFadd{achesâ€, â€œtamiflu informationâ}\DIFaddend ; web pages - \DIFdelbegin \DIFdel{?}\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend Home Remedies for Flu Symptoms (WebMD)\DIFdelbegin \DIFdel{?}\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend ; http://www.rxwiki.com/slideshow/surprising-facts-about-flu/chicken-soup-actually-helps. 
Keep in mind that pages specifying diagnoses and remedy of canonical flu symptoms go in this category.

\item A2. Flu not in the context of experienced probable ILI \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend trends, prevention (including vaccination), policies, scientific background, etc.

Examples: Searches - ``flu epidemic'',``flu vaccine 23\% effective'', ``avian flu strain''; web pages - ``TABLE. Influenza Vaccines \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend United States, \DIFdelbegin \DIFdel{2014?}\DIFdelend \DIFaddbegin \DIFadd{2014â}\DIFaddend 15 Influenza Season*,  Seasonal Influenza (Flu)  \DIFdelbegin \DIFdel{CDC?}\DIFdelend \DIFaddbegin \DIFadd{CDCâ}\DIFaddend ; ``Flu Activity \& Surveillance, Seasonal Influenza (Flu) , CDC''.

\item B1. Secondary flu symptoms (generic or as experienced) \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend sore throat, headache, body aches, fatigue, runny nose, stuffed up nose, vomiting, diarrhea.

Examples: searches - ``sore throat gargle salt water'', ``sore throat and breast pain'', ``3 year old runny nose only''; web pages \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend ``Is It a Cold or a Sinus Infection? Symptoms \& Treatments (WebMD)'', ``Sore Throat Home Remedies and Treatments''.

\item B2. Secondary flu symptoms not in the context of experienced probable ILI 

Examples: searches \DIFdelbegin \DIFdel{? ?}\DIFdelend \DIFaddbegin \DIFadd{â€“ â}\DIFaddend tiger vomits on \DIFdelbegin \DIFdel{course?, ?holiday vomit fest?, ?}\DIFdelend \DIFaddbegin \DIFadd{courseâ€, â€œholiday vomit festâ€, â}\DIFaddend pres. obama sore \DIFdelbegin \DIFdel{throat?
}\DIFdelend \DIFaddbegin \DIFadd{throatâ}\DIFaddend 

 \item C1. Other generic or experienced illness syndromes or symptoms \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend croup, pneumonia, stomach flu, ear infection, etc.

Examples: searches \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend ``can adults get croup'', ``whooping cough'', ``swollen toe'', ``cdc ebola''; web pages \DIFdelbegin \DIFdel{? }\DIFdelend \DIFaddbegin \DIFadd{â}\DIFaddend \url{http://www.webmd.com/digestive-disorders/diarrhea-stomach-flu}, ``Middle ear mucus won't drain. Ear, Nose \& Throat Community - Support Group (WebMD)''

\item D. Unrelated to human illness 

Examples: searches \DIFdelbegin \DIFdel{? ?cat cough?, ?frozen fever? ?}\DIFdelend \DIFaddbegin \DIFadd{â€“ â€œcat coughâ€, â€œfrozen feverâ€ â}\DIFaddend acdc rock or bust \DIFdelbegin \DIFdel{tour?, ?marisa coughlan feet?, ?}\DIFdelend \DIFaddbegin \DIFadd{tourâ€, â€œmarisa coughlan feetâ€, â}\DIFaddend saturday night fever \DIFdelbegin \DIFdel{soundtrack?}\DIFdelend \DIFaddbegin \DIFadd{soundtrackâ}\DIFaddend ; web pages \DIFdelbegin \DIFdel{? ?}\DIFdelend \DIFaddbegin \DIFadd{â€“ â}\DIFaddend Pflugerville, Texas - Wikipedia, the free \DIFdelbegin \DIFdel{encyclopedia?; ?}\DIFdelend \DIFaddbegin \DIFadd{encyclopediaâ€; â}\DIFaddend Influence and reception of \DIFdelbegin \DIFdel{Søren }\DIFdelend \DIFaddbegin \DIFadd{SÃ¸ren }\DIFaddend Kierkegaard - Wikipedia, the free \DIFdelbegin \DIFdel{encyclopedia?
}\DIFdelend \DIFaddbegin \DIFadd{encyclopediaâ}\DIFaddend \end{itemize}

\DIFdelbegin \subsection*{\DIFdel{Labeling Queries and Pages from Respondent Browsing History}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Labeling Queries and Pages from Respondent Browsing History}}
\DIFaddend 
...
\section{\DIFadd{Query Data Geographic and Temporal Coverage}}

\DIFadd{Search query data provided in partnership with Bing cover years from 2012 to 2017 for all 50 states plus the District of Columbia. While we are unable to share overall rates of searches from Bing, we can share relative coverage rates for the states and years included. For the state rates, we counted the number of queries in each year, then normalized by population to generate a query rate per capita measure. We then subtracted the state average and divided by the standard error to generate a z-score. We did this separately for the overall query count, flu queries (queries that were flagged as likely related to flu based on our DOC2VEC expansion), and the A1 query count. 
}

\DIFadd{For the yearly rates, we calculated the average query count in each year, then subtracted the overall yearly mean and divided by the standard deviation to arrive at the z-score. The figures below show the calculated z-scores by state and year (see figures \ref{state_coverage} and \ref{year_coverage}). 
}

\begin{figure}[!htbp]
\begin{centering}
   \includegraphics[width=1\textwidth]{state_coverage.pdf}
  \caption{\DIFaddFL{State coverage of Bing queries 2012-2017}}
...
\begin{centering}
  ...
  \caption{\DIFaddFL{Yearly coverage of Bing queries 2012-2017}}
\label{year_coverage}
\end{centering}
\end{figure}

\DIFaddend \subsection{Predicting Internet Behavior Based on Reported Symptoms}
...
We find a heightened tendency to make both A1 and A2  searches/visits when there is an occurrence of flu symptoms in the household. In figure 1 in the main paper, the means of $ = Pr(Y=1|x_1, \pi)$ and $ Pr(Y=1|x_0, \pi) $ are shown with dark vertical lines. The estimated Risk Ratio (RR) is 1.57 \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(95\% CI = 1.05, 2.34), }\DIFaddend meaning that those with flu symptoms execute almost 60\% more A1 searches compared to those exhibiting no symptoms at all (see table ~\ref{table:coefficients_a1}). However, since the base rate is very low, this amounts to a Risk Difference (RD) of about 5.41e-06 \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(95\% CI = 5.57e-07, 1.09e-05), }\DIFaddend which relates to a very small change in the overall probability of an A1 search given flu symptoms. Nonetheless, the risk difference is statistically significant (p$<$.05). 
...
}\DIFdelend \DIFaddbegin \DIFadd{$Pr(y_i = 1) = logit^{-1}( \beta_0  + \beta_{[i]}^{Income}
+ \alpha_{j[i]}^{State}  
...
%DIFDELCMD < %%%
\DIFdel{$\alpha_p^{Age} \sim  N (0, \sigma_{Age}^2), for l = 1, ..., 4 $ }%DIFDELCMD < \\
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdelend %DIF > \noindent
%DIF > $\alpha_j^{State} \sim  N (0, \sigma_{State}^2), \forall h = 1, ...., 52  $ \\
%DIF > $\alpha_k^{Education} \sim  N (0, \sigma_{Education}^2), \forall q = 1, ..., 4  $ %\\
%DIF > $\alpha_p^{Age} \sim  N (0, \sigma_{Age}^2), for l = 1, ..., 4 $ \\
%DIF > $\alpha_q^{Child-per-house} \sim  N (0, \sigma_{Child-per-house}^2), \forall s = %1, ..., 5  $ \\
%DIF > $\alpha_{k,p}^{Education*Age} \sim  N (0, \sigma_{Education*Age}^2), \forall k = %1, ..., 4 \textit{ and } p = 1,...,4  $ \\

Where $\beta_0$ is the fixed baseline intercept of the fraction of A1 queries over all queries matching our sample. 

$\beta_{1[i]}^{Income}$ is a fixed coefficient corresponding to the median income in that zip code (ACS 2016 one-year data). We chose this predictor because there is evidence that income is associated with an individual's relationship with the Internet (\url{http://www.pewinternet.org/fact-sheet/internet-broadband/}). We \DIFdelbegin \DIFdel{also }\DIFdelend wanted at least one fixed predictor in order to improve the performance of \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend MRP model \citep{buttice_and_highton_2013}.
...
\DIFaddend %As an additional test estimate, we also construct a `smoothed MRP' estimate which utilizes a non-parametric temporal loess smoother to further reduce signal noise (Wang and Ye, 2010). For a comparison estimate, we also create a simple sum of A1 searches on each day in each state, which we denote the `raw' estimate. 


\subsection{More Discussion of Forecasting Results}

The following \DIFdelbegin \DIFdel{models were then considered }\DIFdelend \DIFaddbegin \DIFadd{forecasting models are considered in this paper:
}\DIFaddend \begin{itemize}
\item \DIFdelbegin \DIFdel{SARIMA-HIST - $\phi_p(B)\Phi_P(B^s)Y^{\text{ILI}}(t) = \theta_q(B)\Theta_Q(B^s)\epsilon(t)$ }\DIFdelend \DIFaddbegin \DIFadd{SARIMA-X }\\
\DIFadd{$\phi_p(B)\Phi_P(B^s)Y^{\text{ILI}}(t) =  \theta_q(B)\Theta_Q(B^s)\epsilon(t) + I_X\phi_1 X_{\text{mrp}}(t)$
}\DIFaddend \item \DIFdelbegin \DIFdel{SARIMA-MRP - $\phi_p(B)\Phi_P(B^s)Y^{\text{ILI}}(t) =  \theta_q(B)\Theta_Q(B^s)\epsilon(t) + \phi_1 X_{\text{mrp}}(t)$ 
}%DIFDELCMD < \item %%%
...
\DIFdelend LASSO-A1 \DIFdelbegin \DIFdel{- $Y^{\text{ILI}}(t) = \sum_{i=1}^p \theta_i Y^{\text{ILI}}(t-i) +  \sum_{j=1}^m \phi_j X_j^{\text{A1}}(t) + \epsilon(t)$
}\DIFdelend \DIFaddbegin \\
\DIFadd{$Y^{\text{ILI}}(t) = \sum_{i=1}^p \theta_i Y^{\text{ILI}}(t-i) +  I_X\sum_{j=1}^m \phi_j X_j^{\text{A1}}(t) + \epsilon(t)$
}\DIFaddend \end{itemize}
\DIFaddbegin 

\DIFaddend for the seasonal ARIMA model with exogenous variables (\DIFdelbegin \DIFdel{SARIMA-MRP/SARIMA-A1) or without (SARIMA-HIST) the notation }\DIFdelend \DIFaddbegin \DIFadd{SARIMA-X) the notation in this paper }\DIFaddend refers to a ARIMA$(p,d,q)\times(P,D,Q)_s$ model in the Box-Jenkins terminology \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{box_etal_2015}}%DIFAUXCMD
. The }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{box_etal_2015}}%DIFAUXCMD
. We estimate the }\DIFaddend LASSO-A1 \DIFdelbegin \DIFdel{/LASSO-HIST models were estimated }\DIFdelend \DIFaddbegin \DIFadd{model }\DIFaddend using a Lasso penalty. \DIFaddbegin \DIFadd{This is essentially a class of models referred to as AR-X (Auto-regressive with exogenous). }\DIFaddend ARGO \cite{yang_etal_2015inference, yang_etal_2015} is an example \DIFaddbegin \DIFadd{of this }\DIFaddend in the context of ILI prediction. \DIFaddbegin \DIFadd{The indicator variable $I_X$ refers to the inclusion ($I_X=1$) or exclusion ($I_X=0$) of exogenous signals. Pure history based models with $I_X=0$ serve as a benchmark we compare our models to (no exogenous signals). 
}\DIFaddend 

\DIFdelbegin \DIFdel{For SARIMA based models we chose }\DIFdelend \DIFaddbegin \DIFadd{For SARIMA-X we selected }\DIFaddend the appropriate orders ($p,P,q,Q$) for the model using AIC as a criterion. We set $p=52$ in the \DIFdelbegin \DIFdel{LASSO based models }\DIFdelend \DIFaddbegin \DIFadd{Lasso-A1 model }\DIFaddend to account for \DIFdelbegin \DIFdel{the seasonal effect }\DIFdelend \DIFaddbegin \DIFadd{seasonal effects }\DIFaddend in ILI rates\DIFdelbegin \DIFdel{, but, since we were }\DIFdelend \DIFaddbegin \DIFadd{. However, since we are }\DIFaddend using the Lasso penalty \DIFdelbegin \DIFdel{, }\DIFdelend the coefficients of most of these lags \DIFdelbegin \DIFdel{did }\DIFdelend \DIFaddbegin \DIFadd{will }\DIFaddend not appear in the final model. This approach also \DIFdelbegin \DIFdel{served }\DIFdelend \DIFaddbegin \DIFadd{serves }\DIFaddend to select the appropriate lag in modeling ILI rates. Finally, we \DIFdelbegin \DIFdel{provided }\DIFdelend \DIFaddbegin \DIFadd{provide }\DIFaddend predictions for the $2015-2017$ seasons by using a rolling three year period to train and predict.
...
%DIFDELCMD < %%%
\DIFdel{It apperars }\DIFdelend \DIFaddbegin \DIFadd{It appears }\DIFaddend from Figure \ref{fig:State_Abs_Errs} that incorporating the MRP signal in the SARIMA-X model controls the variance in the prediction error (the absolute errors are less spiky) . It seems as a general approach for predicting time dependent metrics, the inclusion of appropriate exogenous signals can have a considerable beneficial impact on the prediction quality.
