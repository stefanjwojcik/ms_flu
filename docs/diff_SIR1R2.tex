...
%DIF DEL SI_tex_R1.tex   Sun Apr 26 13:13:45 2020
%DIF ADD SI_tex_R2.tex   Sun Apr 26 13:28:21 2020
...
\DIFdelbegin \DIFdel{$Pr(y_i = 1) = logit^{-1}( \beta_0  + \beta_{[i]}^{Income}
+ \alpha_{j[i]}^{State}
+ \alpha_{j[i]}^{Education}
+ \alpha_{j[i]}^{Age} \\
+ \alpha_{j[i]}^{Child-per-House}
+ \alpha_{j[i]}^{Education*Age} $
}\DIFdelend \DIFaddbegin \DIFadd{$Pr(y_i = 1) = logit^{-1}( \beta_0  + \beta_{[it]}^{Income}
+ \alpha_{j[it]}^{State}
...
\DIFaddbegin \subsection*{\DIFadd{State Level Findings}}
...
%DIFAUXCMD
\DIFdel{It appears from Figure \ref{fig:State_Abs_Errs} }\DIFdelend \DIFaddbegin 

\DIFadd{Figure \ref{fig:State_Abs_Errs} shows }\DIFaddend that incorporating the MRP signal in the SARIMA-X model controls the variance in the prediction error (the absolute errors are less spiky) . \DIFdelbegin \DIFdel{It seems as }\DIFdelend \DIFaddbegin \DIFadd{These plots visually represent what is presented in Table 3 in the state level RMSE results. As }\DIFaddend a general approach for predicting time dependent metrics, the inclusion of appropriate exogenous signals \DIFaddbegin \DIFadd{such as the MRP signal }\DIFaddend can have a considerable beneficial impact on the prediction quality.
