...
%DIF DEL main_tex_R2.tex   Sat Sep  5 13:08:39 2020
%DIF ADD main_tex_R3.tex   Tue Oct  6 00:44:06 2020
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
%DIF 4a4-6
\usepackage{endnotes} %DIF > 
\usepackage[justification=centering]{subfig} %DIF > 
 %DIF > 
%DIF -------
...
%DIF 7c10
%DIF < \author[b,+]{Avleen Bijral} 
%DIF -------
\author[b,+]{Avleen S. Bijral}  %DIF > 
%DIF -------
\author[b]{Richard Johnston}
%DIF 9c12
%DIF < \author[b]{Juan Miguel Lavista}
%DIF -------
\author[b]{Juan M. Lavista Ferres} %DIF > 
%DIF -------
\author[c]{Gary King}
...
To identify \DIFdelbegin \DIFdel{`}\DIFdelend A1 \DIFdelbegin \DIFdel{' flu-like }\DIFdelend \DIFaddbegin \DIFadd{(flu-related) }\DIFaddend searches and browsing behavior from respondent data, we asked trained human coders to label each search query and web page visited by respondents in a multi-step process (\DIFdelbegin \DIFdel{SI: }\DIFdelend \DIFaddbegin \DIFadd{Supplementary Information }\DIFaddend 5.4). We found 21\% of respondents made an A1 query or page visit, and 14\% made an A2 query or page visit. With these labels in hand, we modeled the relationship between reported flu-like symptoms and online activity.
...
%DIFDELCMD < \item %%%
\DIFdel{SARIMA-HIST - $\phi_p(B)\Phi_P(B^s)Y^{\text{ILI}}(t) = \theta_q(B)\Theta_Q(B^s)\epsilon(t)$ }%DIFDELCMD < \item %%%
...
\begin{table}[tbp]
\centering
\DIFaddbeginFL \textbf{\underline{\DIFaddFL{Table National ILI Accuracy}}}\par\medskip
\DIFaddendFL \begin{tabular}{|l|l|l|l|}
\hline
...
\end{tabular}
\caption{National ILI Accuracy (horizon = 1 week) scores based on percent CDC ILI visits. The rows indicate a model loss - root mean squared error (RMSE), mean average percent error (MAPE), and mean average error (MAE). See \DIFdelbeginFL \DIFdelFL{SI:}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Supplementary Information }\DIFaddendFL 6.8 for loss calculations. \DIFaddbeginFL \DIFaddFL{Bold text with asterisk indicates minimum values.}\DIFaddendFL }
\label{table:mse-national}
\end{table}

 \begin{figure}%[tbhp]
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.51\linewidth]{USBox_h=1}%%%
\DIFdelendFL \DIFaddbeginFL \textbf{\underline{\DIFaddFL{National Tracking Model Performance Metrics}}}\par\medskip
    \includegraphics[width=.45\linewidth]{USBox_h=1}\DIFaddendFL \\
        \DIFaddbeginFL \subfloat[\label{boxp} Absolute Error boxplot.]{\hspace{.3\linewidth}}\\
 \DIFaddendFL \includegraphics[width=.45\linewidth]{US_Predictions_h=2}&
  \includegraphics[width=.45\linewidth]{US_Predictions_h=1}\DIFaddbeginFL \\
  \subfloat[\label{2step} $h=2$ step predictions.]{\hspace{.3\linewidth}}
\subfloat[\label{1step} $h=1$ step predictions.]{\hspace{.3\linewidth}}
 \DIFaddendFL \caption{a) \DIFdelbeginFL \DIFdelFL{The }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Box and Whisker plot of the }\DIFaddendFL absolute error of multiple model predictions based on the national percent of ILI visits based on CDC data \DIFaddbeginFL \DIFaddFL{(T=123 weeks)}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Horizontal line is median, boxes indicate 25\% and 75\% quantiles, and whiskers extend no more than 1.5 the length of each box. }\DIFaddendFL b)\DIFaddbeginFL \DIFaddFL{/c) }\DIFaddendFL The x-axis depicts the 2017 date and the y-axis depicts the predicted/true national percent of ILI visits per CDC data. Solid line is the actual CDC ILI visits and the dotted lines are \DIFdelbeginFL \DIFdelFL{model predictions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{point forecasts }\DIFaddendFL based on the SARIMA-HIST and SARIMA-MRP models along with \DIFdelbeginFL \DIFdelFL{confidence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{upper and lower prediction }\DIFaddendFL intervals for both $h=1$ and $h=2$ steps ahead. \DIFaddbeginFL \DIFaddFL{(T=123 weeks)}\DIFaddendFL }
  \label{fig:US_Box}
 \end{figure}

%DIF < \begin{figure}%[tbhp]
%DIF <  \centering
%DIF <  \includegraphics[width=.8\linewidth]{NY_h_1}\\
%DIF <  \includegraphics[width=.8\linewidth]{DE_h_1}
%DIF <  \caption{a) h=1 Step ahead predictions for New York and Delaware. Unless we use the %MRP signal the predictions overshoot.}
%DIF <   \label{fig:State_Preds}
%DIF <  \end{figure}
\DIFdelbegin %DIFDELCMD < 
...
%DIF <  \begin{figure}%[tbhp]
%DIF <  \centering
%DIF <  \includegraphics[width=.8\linewidth]{DE_NY_fcast_H1}
%DIF <  %\includegraphics[width=.8\linewidth]{USBox_h_2}
%DIF <  \label{fig:DE_NY}
%DIF <  \caption{**NEEDS A CAPTION}
%DIF <  \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Table ~\ref{table:mse-national} shows that the MRP signal improves upon the one-week ahead MAPE (mean average percent error), RMSE (root mean squared)  and MAE (mean absolute error) compared to the history-based models (\DIFaddbegin \DIFadd{for a total period of $T=123$ weeks). (}\DIFaddend all metrics look at the average deviations of model predictions, see \DIFdelbegin \DIFdel{SI:}\DIFdelend \DIFaddbegin \DIFadd{Supplementary Information }\DIFaddend 6.8 for mathematical definitions of these metrics). As such, estimates in 2016/2017 deviated by $12.29/13.74\%$ from the true values in these tests. For reference to a more common metric, the MRP tracking estimate correlates at $0.90/0.84$ with the national ILI rate in the 2016/2017 season. \DIFdelbegin \DIFdel{Additionally, Figure \ref{fig:US_Box}-a) }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{boxp} }\DIFaddend shows that the absolute error for the MRP signal has a lower mean and variance than other history or A1-based models. On average, the SARIMA-MRP model showed fewer outliers and a smaller median error compared to the pure history model.  \DIFdelbegin %DIFDELCMD < \removed{The box plot reveals fewer outliers and a smaller median error for the MRP model compared to the historical and the raw signal based models.} %%%
\DIFdelend We conjecture that since Bing is only a portion of all search queries, our models would perform better with more search data. Additionally if we look at \DIFdelbegin \DIFdel{$2$}\DIFdelend \DIFaddbegin \DIFadd{$h=2$}\DIFaddend -week ahead predictions (the prediction at step \DIFdelbegin \DIFdel{$t+2$}\DIFdelend \DIFaddbegin \DIFadd{$t+h$}\DIFaddend , given the CDC data point at step $t$ and the exogenous signal at step \DIFdelbegin \DIFdel{$t+2$}\DIFdelend \DIFaddbegin \DIFadd{$t+h$}\DIFaddend ) , the MRP signal proves to be even more valuable achieving a RMSE/MAPE/MAE of $0.386/12.855/0.263$ vs $0.456/13.584/0.299$ using only history (SARIMA). \DIFdelbegin \DIFdel{Additionally, Figure \ref{fig:US_Box}b) shows }\DIFdelend \DIFaddbegin \DIFadd{Looking at the plot for our forecasts, figures \ref{1step} and \ref{2step} show }\DIFaddend that SARIMA-MRP predictions are better aligned with the observed national ILI rate with tighter \DIFdelbegin \DIFdel{confidence }\DIFdelend \DIFaddbegin \DIFadd{upper and lower prediction }\DIFaddend intervals for both the $1$-step and $2$-step ahead predictions.
\DIFaddbegin \DIFadd{These $95\%$ intervals are given by $\hat{Y}^{\text{ILI}}(t+h) \pm 1.96 \sqrt{\hat{\sigma}_h}$, where $\hat{Y}^{\text{ILI}}(t+h)$ is the point forecast at time step $t+h$ and $\hat{\sigma}_h$ is the variance of the estimated noise terms (assuming Gaussian errors). More details can be found in \mbox{%DIFAUXCMD
\cite{box_etal_2015}}%DIFAUXCMD
.
}

\DIFaddend Pure history-based models can be very powerful if appropriately trained and tested. To illustrate this, we note that the history-only SARIMA-HIST model outperforms all other approaches in tracking national CDC estimates, save for our demographically-sensitive SARIMA-MRP approach. For another recent SARIMA based Influenza model please see \cite{zhang2019predicting}.

%DIF < Supporting our claims in the previous section, we found that incorporating the MRP signal in the SARIMA-X model often prevents the prediction from overshooting. The historical model anticipates a bigger peak while this is not reflected in the MRP-A1 model predictions. This is clearly seen in the $h=2$-step ahead predictions (Figure \ref{fig:US_Box}-b)). Thus, as a general approach for predicting time dependent metrics, the inclusion of appropriate exogenous signals can have a considerable beneficial impact on the prediction quality.
\DIFaddbegin \subsection*{\DIFadd{State Level Performance}}
\DIFaddend 

\DIFdelbegin \subsection*{\DIFdel{State Level Findings}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend The SARIMA-MRP model performed better than history alone in all four states, and in all the cases it was the outright best model (based on error rates). The improvement of the SARIMA-MRP model was especially sharp with a two-week horizon. Table \ref{tab:h1results} displays the root mean squared errors for a horizon of one and two weeks in each state. 
%DIF < Moreover, Figure \ref{fig:State_Abs_Errs} reveals that the MRP signal based predictions prevented overshooting compared to the history signal. Intuitively, we expected the MRP exogenous signal to closely mimic the underlying influenza metric and hence provide a better prediction.

\begin{table}[tbp]
\centering
\caption{\DIFaddbeginFL \DIFaddFL{State-level tracking model RMSE }\DIFaddendFL (h = 1 week/h = 2 weeks)\DIFdelbeginFL \DIFdelFL{RMSE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Bold text indicates minimum values.}\DIFaddendFL }
\label{tab:h1results}
\scalebox{0.9}{
...
\section*{Discussion\DIFdelbegin \DIFdel{and Limitations}\DIFdelend }
...
%DIF < This approach has wide applicability to problems of modeling time series of small to medium length using search query or social media data. 

%DIF < \matmethods{Please describe your materials and methods here. This can be more than one paragraph, and may contain subsections and equations as required. Authors should include a statement in the methods section describing how readers will be able to access the data in the paper. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \subsection*{Subsection for Method}
%DIF < Example text for subsection.
%DIF < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section*{Code and Data Availability}
